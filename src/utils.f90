...
module utils_module
  ...
  subroutine locate( xstar, x, n, h, subp, ipos_left )
    ...
    integer                  , intent(in)    :: n
    real(F64), dimension(0:n), intent(in)    :: x
  ...
  !--------------------------------------------------------------
  ! Solves for a vector U of length Nx the tridiagonal linear
  ! set given by the equation M*U = R ,
  ! where A,B,C and R are input vectors and are not modified.
  ! (alpha,beta) account for cyclic boundary conditions along Nx
  !--------------------------------------------------------------
  subroutine cyclic_r(A,B,C,alpha,beta,R,U,Nx)
    use prec_const

    implicit none
    integer                      :: i, Nx
    real(RKIND)                  :: alpha, beta, fact, gam
    ! vectors of size 0:Nx
    real(RKIND), dimension(0:),  intent(in)    :: A, B, C, R
    real(RKIND), dimension(0:),  intent(inout) :: U
    real(RKIND), dimension(0:Nx)               :: V, Z, BB
    
    gam    = -B(0)
    BB(0)  = B(0) - gam
    BB(Nx) = B(Nx) - alpha*beta/gam
    do i=1,Nx-1
      BB(i) = B(i)
    end do

    call tridiag_r(A,BB,C,R,U,Nx)
    ...
    call tridiag_r(A,BB,C,V,Z,Nx)

    fact = (U(0)+beta*U(Nx)/gam) / (1._RKIND+Z(0)+beta*Z(Nx)/gam)
    do i=0,Nx
      U(i) = U(i) - fact*Z(i)
    end do
  end subroutine cyclic_r
  ...
  !---------------------------------------------------------
  subroutine compute_trapeze_coeff(Nx,dx,integr_coeff)
